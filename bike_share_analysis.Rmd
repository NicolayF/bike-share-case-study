...
mutate(
  ...
  day_of_week = wday(started_at, label = TRUE, abbr = TRUE, , locale = "en"),
...
format_table(cleaning_summary, caption_blue("Data Cleaning Summary"))
...
format_table(distribution, caption_blue("User Type Distribution After Cleaning"))
```
...
format_table(summary_stats, caption_blue("Descriptive Statistics by User Type"))
```
...
summarise(
  ...
  weekend_share = round(mean(day_of_week_num %in% c(1,7)) * 100, 1),
  peak_hour = hour[which.max(table(hour))][1]
)

format_table(summary_table, caption_blue("Key Behavioral Differences Between Rider Types"))
```
---
...
format_table(top_casual_stations, caption_blue("Top 5 Start Stations: Casual Riders"))
format_table(top_member_stations, caption_blue("Top 5 Start Stations: Members"))
...
* **Action:** Program the app to recognize a long-duration casual ride. When they dock their bike, send a push notification: "You rode for X minutes\! That casual ride cost $Y. An annual pass would have made it free! Learn more." This targets the user at their moment of highest engagement.
